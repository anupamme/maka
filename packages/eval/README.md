...
One deviation from Codex cannot be removed, and it is the first thing a reader of these numbers needs. `apply_patch` in Codex is a **freeform tool**: the API is sent a 108-character description and a Lark grammar the decoder is constrained to, so a model on that path cannot emit a malformed envelope at all. The harness registers JSON function tools and has no grammar seam, so this arm is the unconstrained case. What it gives the model instead is upstream's own prose for exactly that case — the `## apply_patch` section of `prompt_with_apply_patch_instructions.md`, the base instructions Codex carried when the format had to be taught rather than enforced — carried as a verbatim file with three substitutions applied over it, each of which a test requires to apply exactly once. That file is upstream's and it is not delivered from there: at the ported commit it is referenced only from a test whose four model cases all declare `expects_apply_patch_description: false`. Instructions come from `models-manager/models.json`, where seven of the eight templates carry no `## apply_patch` section and leave the format to the grammar — but the eighth, `gpt-5.2`, carries a shorter section of its own, same envelope and same worked example. So the format is still taught in prose upstream, and this file is the fuller version of that prose. Either way no shipped configuration matches this arm, since `gpt-5.2` gets the prose *and* the grammar, so the choice was between upstream's own text and text written here. The arm therefore measures a V4A contract delivered as a function tool, not Codex's contract; syntax errors this arm can make are errors Codex's decoder would have prevented, and that is a cost charged to this arm and to no other. What is *not* a cost charged to it is the absence of a read tool. Codex registers no tool that reads file text either. Its handler set at the ported commit is long — shell, `exec_command`, `write_stdin`, `apply_patch`, `update_plan`, permissions, user input, plugin install, context window, time, sleep, tool search, code mode, extension, web, MCP and two generations of multi-agent handlers — and the only one that takes a filesystem path to read is `view_image`, which takes images. Text is read through the shell. So an arm that reads with `cat` is that contract rather than a handicap applied to it, and the bash every arm gets is the same bash.
